...
def test_simple_agent_run_sync():
    agent = SimpleAgent(name='echo_agent', processor_func=lambda inp, ctx: {'echo': inp})
...
def test_simple_dag_workflow_execution():
    ...
    wf_def = WorkflowDefinition(
        ...
        description='',
        steps=[
            WorkflowStep(id='s1', name='step1', step_type=StepType.AGENT, config={'agent_name': 'a1'}, dependencies=[]),
            WorkflowStep(id='s2', name='step2', step_type=StepType.AGENT, config={'agent_name': 'a2'}, dependencies=['s1']),
        ],
    )

    wf = SimpleDAGWorkflow(definition=wf_def, agent_registry={'a1': agent1, 'a2': agent2})
