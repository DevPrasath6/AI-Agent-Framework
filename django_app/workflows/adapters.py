...
def enqueue_workflow_run(run_id: str, workflow_id: str, payload: dict):
    p = _get_producer()
    event = {'type': 'workflow.run.requested', 'run_id': run_id, 'workflow_id': workflow_id, 'payload': payload}
    if p is not None:
...
def enqueue_agent_run(run_id: str, agent_id: str, payload: dict):
    p = _get_producer()
    event = {'type': 'agent.run.requested', 'run_id': run_id, 'agent_id': str(agent_id), 'payload': payload}
    if p is not None:
