...
class AgentViewSet(viewsets.ModelViewSet):
    ...
    def start(self, request, pk=None):
        ...
        run = AgentRun.objects.create(agent=agent, input_payload=payload, status='QUEUED')
    ...
    def register_from_sdk(self, request):
        ...
        a = get_agent(name)
        if a is None:
            return Response({'error': 'agent not found in SDK registry'}, status=status.HTTP_404_NOT_FOUND)
        # Create DB record
        agent = Agent.objects.create(name=getattr(a, 'name', name), description=getattr(a, 'description', ''))
