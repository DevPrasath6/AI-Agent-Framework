...
def run_kafka_worker(bootstrap_servers: str = None, topics=None, group_id: str = 'ai-agent-framework'):
    if KafkaConsumer is None:
        print('kafka-python not available; kafka worker disabled')
        return
    bootstrap_servers = bootstrap_servers or getattr(settings, 'KAFKA_BOOTSTRAP_SERVERS', 'localhost:9092')
