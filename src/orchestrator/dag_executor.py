...
def execute_dag(dag_def: Any, context: Any = None) -> Any:
    ...
    if isinstance(dag_def, SimpleDAGWorkflow):
        if asyncio.iscoroutinefunction(dag_def.execute):
            return _run_async(
                dag_def.execute(
                    context.shared_data.get("workflow_input") if context else None,
                    context,
                )
            )
        return dag_def.execute(
            context.shared_data.get("workflow_input") if context else None, context
        )

    # If a dict is provided, try to construct SimpleDAGWorkflow
    try:
        wf = (
            SimpleDAGWorkflow.from_definition(dag_def)
            if hasattr(SimpleDAGWorkflow, "from_definition")
            else SimpleDAGWorkflow(dag_def)
        )
        ctx = wf.create_execution_context(
            dag_def.get("input") if isinstance(dag_def, dict) else None
        )
        if asyncio.iscoroutinefunction(wf.execute):
            return _run_async(
                wf.execute(
                    dag_def.get("input") if isinstance(dag_def, dict) else None, ctx
                )
            )
        return wf.execute(
            dag_def.get("input") if isinstance(dag_def, dict) else None, ctx
